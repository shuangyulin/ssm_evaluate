...
public class ClassInfoController extends BaseController {
	...
	public void list(String startDate,String classNumber,String className,@ModelAttribute("colleageObj") Colleage colleageObj,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (startDate == null) startDate = "";
		if (classNumber == null) classNumber = "";
		if (className == null) className = "";
		if(rows != 0)classInfoService.setRows(rows);
		List<ClassInfo> classInfoList = classInfoService.queryClassInfo(startDate, classNumber, className, colleageObj, page);
	    /*计算总的页数和总的记录数*/
	    classInfoService.queryTotalPageAndRecordNumber(startDate, classNumber, className, colleageObj);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String startDate,String classNumber,String className,@ModelAttribute("colleageObj") Colleage colleageObj,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (startDate == null) startDate = "";
		if (classNumber == null) classNumber = "";
		if (className == null) className = "";
		List<ClassInfo> classInfoList = classInfoService.queryClassInfo(startDate, classNumber, className, colleageObj, currentPage);
	    /*计算总的页数和总的记录数*/
	    classInfoService.queryTotalPageAndRecordNumber(startDate, classNumber, className, colleageObj);
	    /*获取到总的页码数目*/
	    int totalPage = classInfoService.getTotalPage();
	...
	public void OutToExcel(String startDate,String classNumber,String className,@ModelAttribute("colleageObj") Colleage colleageObj, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(startDate == null) startDate = "";
        if(classNumber == null) classNumber = "";
        if(className == null) className = "";
        List<ClassInfo> classInfoList = classInfoService.queryClassInfo(startDate,classNumber,className,colleageObj);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "ClassInfo信息记录"; 
        String[] headers = { "班级编号","班级名称","所在学院","班主任","开办日期"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<classInfoList.size();i++) {
        	ClassInfo classInfo = classInfoList.get(i); 
        	dataset.add(new String[]{classInfo.getClassNumber(),classInfo.getClassName(),classInfo.getColleageObj().getColleageName(),classInfo.getBanzhuren(),classInfo.getStartDate()});
        }
        /*
