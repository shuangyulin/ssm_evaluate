...
public class ClassInfoService {
    ...
    /*按照查询条件分页查询班级信息记录*/
    public ArrayList<ClassInfo> queryClassInfo(String startDate,String classNumber,String className,Colleage colleageObj,int currentPage) throws Exception { 
     	String where = "where 1=1";
    	if(!startDate.equals("")) where = where + " and t_classInfo.startDate like '%" + startDate + "%'";
    	...
    	if(null != colleageObj &&  colleageObj.getColleageNumber() != null  && !colleageObj.getColleageNumber().equals(""))  where += " and t_classInfo.colleageObj='" + colleageObj.getColleageNumber() + "'";
    	int startIndex = (currentPage-1) * this.rows;
    	return classInfoMapper.queryClassInfo(where, startIndex, this.rows);
    }

    /*按照查询条件查询所有记录*/
    public ArrayList<ClassInfo> queryClassInfo(String startDate,String classNumber,String className,Colleage colleageObj) throws Exception  { 
     	String where = "where 1=1";
    	if(!startDate.equals("")) where = where + " and t_classInfo.startDate like '%" + startDate + "%'";
    	if(!classNumber.equals("")) where = where + " and t_classInfo.classNumber like '%" + classNumber + "%'";
    	if(!className.equals("")) where = where + " and t_classInfo.className like '%" + className + "%'";
    ...
    /*当前查询条件下计算总的页数和记录数*/
    public void queryTotalPageAndRecordNumber(String startDate,String classNumber,String className,Colleage colleageObj) throws Exception {
     	String where = "where 1=1";
    	if(!startDate.equals("")) where = where + " and t_classInfo.startDate like '%" + startDate + "%'";
    	if(!classNumber.equals("")) where = where + " and t_classInfo.classNumber like '%" + classNumber + "%'";
    	if(!className.equals("")) where = where + " and t_classInfo.className like '%" + className + "%'";
