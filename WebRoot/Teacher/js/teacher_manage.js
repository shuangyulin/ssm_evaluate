...
function initTeacherManageTool() {
	teacher_manage_tool = {
		...
		search: function() {
			...
			queryParams["teacherUserName"] = $("#teacherUserName").val();
				...
				$.messager.confirm("确定操作", "您正在要删除所选的记录吗？", function (flag) {
					if (flag) {
						...
						for (var i = 0; i < rows.length; i ++) {
							teacherUserNames.push(rows[i].teacherUserName);
						}
						$.ajax({
							type : "POST",
							url : "Teacher/deletes",
							data : {
								teacherUserNames : teacherUserNames.join(","),
							},
							beforeSend : function () {
		...
		edit : function () {
			...
			if (rows.length > 1) {
				...
				$.ajax({
					url : "Teacher/" + rows[0].teacherUserName +  "/update",
					type : "get",
					data : {
						//teacherUserName : rows[0].teacherUserName,
					},
					beforeSend : function () {
						$.messager.progress({
							text : "正在获取中...",
