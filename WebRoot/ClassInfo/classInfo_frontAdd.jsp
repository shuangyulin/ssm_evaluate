			...
			<div>
				...
				<div class="tab-content">
				    ...
				    <div role="tabpanel" class="tab-pane active" id="classInfoAdd"> 
						  ...
						  <div class="form-group">
						  	 <label for="classInfo_colleageObj_colleageNumber" class="col-md-2 text-right">所在学院:</label>
						  	 <div class="col-md-8">
							    <select id="classInfo_colleageObj_colleageNumber" name="classInfo.colleageObj.colleageNumber" class="form-control">
							    </select>
						  	 </div>
						  </div>
						  <div class="form-group">
						  	 <label for="classInfo_banzhuren" class="col-md-2 text-right">班主任:</label>
						  	 <div class="col-md-8">
							    <input type="text" id="classInfo_banzhuren" name="classInfo.banzhuren" class="form-control" placeholder="请输入班主任">
							 </div>
						  </div>
						  <div class="form-group">
						  	 <label for="classInfo_startDateDiv" class="col-md-2 text-right">开办日期:</label>
						  	 <div class="col-md-8">
				                <div id="classInfo_startDateDiv" class="input-group date classInfo_startDate col-md-12" data-link-field="classInfo_startDate" data-link-format="yyyy-mm-dd">
				                    <input class="form-control" id="classInfo_startDate" name="classInfo.startDate" size="16" type="text" value="" placeholder="请选择开办日期" readonly>
...
$(function(){
	...
	$.ajax({
		...
		success: function(colleages,response,status) { 
			$("#classInfo_colleageObj_colleageNumber").empty();
			var html="";
    		$(colleages).each(function(i,colleage){
    			html += "<option value='" + colleage.colleageNumber + "'>" + colleage.colleageName + "</option>";
    		});
    		$("#classInfo_colleageObj_colleageNumber").html(html);
    	}
	});
	//开办日期组件
	$('#classInfo_startDateDiv').datetimepicker({
		language:  'zh-CN',  //显示语言
		format: 'yyyy-mm-dd',
		minView: 2,
		weekStart: 1,
		...
		$('#classInfoAddForm').data('bootstrapValidator').updateStatus('classInfo.startDate', 'NOT_VALIDATED',null).validateField('classInfo.startDate');
	});
})
</script>
</body>
