...
$(function () { 
	...
	$("#colleage_manage").datagrid({
		...
		pageNumber : 1,
		sortName : "colleageNumber",
		sortOrder : "desc",
		toolbar : "#colleage_manage_tool",
		columns : [[
			{
				field : "colleageNumber",
				title : "学院编号",
				width : 140,
			},
			{
				...
				if ($("#colleageEditForm").form("validate")) {
					...
					if(!$("#colleageEditForm").form("validate")) {
						...
						$("#colleageEditForm").form({
						    url:"Colleage/" + $("#colleage_colleageNumber_edit").val() + "/update",
						    onSubmit: function(){
								if($("#colleageEditForm").form("validate"))  {
				                	$.messager.progress({
										text : "正在提交数据中...",
...
function initColleageManageTool() {
	colleage_manage_tool = {
		...
		search: function() {
			var queryParams = $("#colleage_manage").datagrid("options").queryParams;
			queryParams["colleageNumber"] = $("#colleageNumber").val();
			queryParams["colleageName"] = $("#colleageName").val();
			$("#colleage_manage").datagrid("options").queryParams=queryParams; 
				...
				$.messager.confirm("确定操作", "您正在要删除所选的记录吗？", function (flag) {
					if (flag) {
						...
						for (var i = 0; i < rows.length; i ++) {
							colleageNumbers.push(rows[i].colleageNumber);
						}
						$.ajax({
							type : "POST",
							url : "Colleage/deletes",
							data : {
								colleageNumbers : colleageNumbers.join(","),
							},
							beforeSend : function () {
		...
		edit : function () {
			...
			if (rows.length > 1) {
				...
				$.ajax({
					url : "Colleage/" + rows[0].colleageNumber +  "/update",
					type : "get",
					data : {
						//colleageNumber : rows[0].colleageNumber,
					},
					beforeSend : function () {
						$.messager.progress({
							text : "正在获取中...",
					...
					success : function (colleage, response, status) {
						...
						if (colleage) { 
							...
							$("#colleage_colleageNumber_edit").validatebox({
								required : true,
